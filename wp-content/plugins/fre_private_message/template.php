	...
	function ae_private_message_button($bid, $project)
	{
		...
		if ($user_ID == (int)$project->post_author && $project->post_status == 'publish') {
			if( $response['success'] ){
				...
				?>
                <a class="probtnsabt fre-normal-btn-o btn-send-msg btn-open-msg-modal" href="javascript:void(0)"><?php _e('Contact',ET_DOMAIN) ?>
                    <script type="data/json"  class="privatemsg_data">
                        <?php  echo json_encode( $data ) ?>
                    </script>
                </a>
			<?php }else{ ?>
                <a class="probtnsabt fre-normal-btn-o btn-send-msg btn-redirect-msg" href="javascript:void(0)"  data-conversation="<?php echo $response['conversation_id'] ?>">
                        ...
                        <div>
                            <form role="form" id="private_msg_form" class="fre-modal-form" >
                                ...
                                <div class="fre-input-field">
                                    ...
                                    <textarea class="fieldechange" name="post_content" id="" cols="30" rows="10"></textarea>   <!-- mene77  fieldechange -->
                                </div>

                                <div class="fre-form-btn">
                                    <button type="submit" class="probtnsabt fre-normal-btn  btn-send-msg-modal"><?php _e('Send', ET_DOMAIN); ?></button>
	...
	function ae_private_message_add_profile_tab_content_template(){
        ...
        <div class="fre-page-wrappe conversation-panelr" id="tab_private_msg">
            <div class="fre-page-title">
                <div class="container">
                    <h2><?php _e('Private Message', ET_DOMAIN ) ?></h2>
                </div>
            </div>
                            ...
                            <div class="col-md-4 private-message-conversation-contents ">
                                <div class="inbox-user-wrap">
                                    <div class="search-inbox-user">
                                        <?php $placeholder_search = _x('Search freelancer name',ET_DOMAIN);
                                        global $user_ID;
                                        $user_role = ae_user_role($user_ID);
                                        if($user_role == 'freelancer'){
                                            $placeholder_search = _x('Search employer name',ET_DOMAIN);
                                        }
                                        ?>
									...
									if( have_posts() ):
                                        ...
                                        echo '<div class="no-message-wrap"><p>';
										_e('You have not created any conversation yet. Please come back to your bidders list in Project detail for starting a conversation with bidders.', ET_DOMAIN);
										echo '</p></div>';
