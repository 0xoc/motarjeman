...
class FRE_Credit_History extends AE_Posts
{
    ...
    public function  fre_credit_register_post_type(){
        ...
        register_post_type('fre_credit_history', array(
            ...
            'labels' => array(
                'name' => 'تاریخچه اعتبار' ,
                'singular_name' => 'تاریخچه اعتبار' ,
                'add_new' => 'افزودن',
                'add_new_item' => 'افزودن تاریخچه اعتبار جدید',
                'edit_item' => 'ویرایش تاریخچه اعتبار',
                'new_item' => 'تاریخچه اعتبار جدید',
                'all_items' =>'همه تاریخچه اعتبارات',
                'view_item' => 'مشاهده تاریخچه اعتبار',
                'search_items' => 'جستجوی تاریخچه اعتبار',
                'not_found' => 'تاریخچه اعتباری یافت نشد',
                'not_found_in_trash' => 'هیچ تاریخچه اعتباری در سطل زباله یافت نشد',
                'parent_item_colon' => '',
                'menu_name' => 'تاریخچه اعتبار',
            ) ,
//            'labels' => array(
...
class Fre_Credit_HistoryAction extends AE_PostAction
{
    ...
    public function fre_credit_convert_history($result){
        ...
        switch($post_title){
            case 'Refunded':
                ...
                $result->post_title = __('Refunded',ET_DOMAIN);
                break;
            case 'withdrew':
            ...
            case 'Paid':
	            ...
	            $payment_gateway = __('Credit system',ET_DOMAIN);
	            $result->post_title = __('Pay',ET_DOMAIN);
                break;
            case 'Received':
                if(!empty($payment)){
                    $project = get_post($payment);
                ...
                $result->post_title = __('Receive',ET_DOMAIN);
                break;
        }
    ...
    public function filter_query_args($query_args){
        ...
        if( isset($_REQUEST['query']['fre_credit_from']) && !empty($_REQUEST['query']['fre_credit_from']) ){
            ...
            $gdate=jalali_to_gregorian_for_filter_credit($_REQUEST['query']['fre_credit_from']);
            $str_from = $gdate . ' 00:00';
        ...
        if( isset($_REQUEST['query']['fre_credit_to']) && !empty($_REQUEST['query']['fre_credit_to']) ){
            ...
            $gdate=jalali_to_gregorian_for_filter_credit($_REQUEST['query']['fre_credit_to']);
            $str_to = $gdate . ' 23:59';
