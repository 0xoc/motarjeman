...
function credit_get_total_project_payment($user_ID){
	...
	if(!empty($list_project_payment)){
		foreach ($list_project_payment as $v){
			...
			$payment_amount= 0;
			if(!empty($fre_paykey)){
				$payment_amount = get_post_meta($fre_paykey,'amount',true);
			}
			$total_project_payment += $payment_amount;
		}
	}
    ...
    function fre_mail_notify_admin_has_new_deposit($message, $product, $order)   {
        ...
        if( $type == 'fre_credit_plan' ){
            ...
            if(ae_get_option('fre_credit_deposit_mail_template')){
                ...
                $message = "<p>سلام مدیر,</p>
                        <p>کاربر [display_name] مبلغ [number] در سایت شما سپرده کرد,لطفا پرداخت را بررسی و در صورت نیاز تایید نمایید</p>
                        <p>با احترام,</p>
                        <p>[blogname]</p>";
            }

            $message = str_ireplace('[number]', $number_of_post, $message);
        }
