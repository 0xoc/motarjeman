    ...
    public function fre_credit_process_payment($payment_return, $data ){
        ...
        if( isset($payment_return['payment_status']) ){
            ...
            if($sku=='custom'){
                //add by mln
                $init = new stdClass;
                $init->post_title = "دلخواه";
                $init->post_content = "<p>در این بسته شما شارژ مورد نظر خود را به دلخواه وارد میکنید</p>";
                $init->post_type = "fre_credit_plan";
                $init->post_status = "publish";
                $init->sku = "custom";
                $init->et_price = $order_pay['total'];
                $init->et_number_posts = $order_pay['total'];
                $init->option_name = "fre_credit_plan";
                $pack=$init;
            }else{
                $pack = $packs->get_pack($sku, 'fre_credit_plan');
            }
            //end add by morteza up

            if($pack != false){
                // comment from 1.2.2 version by danng
