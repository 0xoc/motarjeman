...
Class Fre_Mailing extends AE_Mailing {
	...
	function new_payment_notification( $order_id ) {
		...
		//fa_v1.8.6.2
		do_action('in_new_payment_notification',$product,$author);
	...
	function bid_mail( $bid_id ) {
		...
		if ( $author ) {
			...
			//fa_v1.8.6.2
			do_action('in_bid_mail',$bid_msg,$author,$project_id,$post_author);
	...
	function review_freelancer_email( $project_id ) {
		...
		//fa_v1.8.6.2
		do_action('in_review_freelancer_email',$post_link,$employer,$project_id,$freelancer_id);
	...
	function review_employer_email( $project_id ) {
		...
		//fa_v1.8.6.2
		do_action('in_review_employer_email',$employer,$project_id,$link_profile,$employer_id);
	...
	function invite_mail( $user_id, $project_id ) {
		...
		if ( $user_id && $project_id ) {
			...
			//fa_v1.8.6.2
			do_action('in_invite_mail',$project_info,$user_id,$value);
	...
	function bid_accepted( $freelancer_id, $project_id ) {
		...
		//fa_v1.8.6.2
		do_action('in_bid_accepted',$workspace_link,$freelancer_id,$project_id);
	...
	function new_message( $receiver, $project, $message ) {
		...
		//fa_v1.8.6.2
		do_action('in_new_message',$message,$workspace_link,$receiver,$project);
	...
	function notify_execute( $project_id, $bid_accepted ) {
		...
		//fa_v1.8.6.2
		do_action('in_notify_execute',$employer,$link_project,$bid_owner,$project_owner,$project_id);
	...
	function alert_transfer_money( $project_id, $bid_accepted ) {
		...
		if ( ! ae_get_option( 'manual_transfer' ) ) {
			...
			$subject        = "پرداخت با موفقیت ارسال شد";
                $admin_template = sprintf( '<p>سلام,</p>
                                        <p>پروژه با عنوان %s با موفقیت پایان یافت و مبلغ با موفقیت به فریلنسر %s ارسال شد.</p>
                                        <p>شما می توانید نظرتان را در مورد پروژه در لینک زیر بنویسید:</p>
                                        <p>%s</p>
			...
			$subject        = "ارسال دستی پول به فریلنسر";
                $admin_template = sprintf( "<p>سلام,</p>
                                        <p>پروژه با عنوان %s با موفقیت توسط کارفرما %s به کامل شده تغییر وضعیت داده شد</p>
                                        <p>لطفا دیدگاه خود را در مورد این پروژه در لینک زیر نوشته و مبلغ را به فریلنسر %s ارسال نمایید: </p>
                                        <p>%s</p>
                                        <p>با احترام, <br>[blogname]</p>",
	...
	function new_project_of_category( $project ) {
		...
		if(!empty($project->tax_input['skill'])){
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					...
					if ( ! empty( $display_name ) && ! empty( $user_email ) ) {
						...
						//fa_v1.8.6.2
						if(function_exists('get_mobiles_for_new_project_of_category')){
							$mobiles[]=get_mobiles_for_new_project_of_category($convert->post_author);
						}
			...
			$template_default = "<p>سلام,</p><p>امروز کار جدید برای شما وجود دارد. پس برای درخواست به این پروژه عجله کنید:[project_link]</p><p>با آروزی یک روز پر برکت برای شما</p>";
			...
			//fa_v1.8.6.2
			do_action('in_new_project_of_category',$link,$mobiles);
