...
class AppEngine extends AE_Base {
	...
	public function print_scripts() {
		...
		//fa_v1.8.6.2
		$maxFileSize=ae_get_option( 'maxSizeFile' ) ;
		if(!empty($maxFileSize) && $maxFileSize>0){
			$max_upload_size = 1024*$maxFileSize;
        }else{
		...
		if ( ! is_admin() ) {
		    //add by morteza
		    if(is_rtl()){
                $this->register_style( 'bootstrap', ae_get_url() . '/assets/css/bootstrap.min-rtl.css', array(), '3.0' );
            }else{
