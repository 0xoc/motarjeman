			...
			<select name="role" class="role-change regular-input" >
				<?php foreach ( $role_names as $role_name => $role_label ) {
					if($role_name == $user_role )
					    //add role_to_farsi by morteza
						echo '<option value="'. $role_name .'" selected="selected">'. role_to_farsi($role_label) .'</option>';
					else
                        //add role_to_farsi by morteza
						echo '<option value="'. $role_name .'" >'. role_to_farsi($role_label) .'</option>';
				} ?>
			</select>
			<?php
