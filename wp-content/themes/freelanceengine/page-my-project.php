                                            ...
                                            <div class="row">
                                                <div class="col-md-8 col-sm-6 col-xs-12">
                                                    <div class="fre-input-field">
                                                        ...
                                                        <input type="text" class="fieldechange search" name="s"
                                                               placeholder="<?php _e( 'Search projects by keywords', ET_DOMAIN ); ?>">  <!-- mene77  fieldechange -->
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="fre-table-row">
                                                        ...
                                                        <div class="fre-table-col project-action-col">
															...
															if ( $bid_status == 'accept' ) {
																echo '<a class="probtnsabt" href="' . add_query_arg( array( 'workspace' => 1 ), $convert->project_link ) . '" target="_blank">' . __( 'Workspace', ET_DOMAIN ) . '</a>';
																...
																echo '<a class="probtnsabt" href="' . add_query_arg( array( 'dispute' => 1 ), $convert->project_link ) . '" target="_blank">' . __( 'Dispute Page', ET_DOMAIN ) . '</a>';
                                        ...
                                        <form>
                                            <div class="row">
                                                <div class="col-md-8 col-sm-6 col-xs-12">
                                                    <div class="fre-input-field">
                                                        <label class="fre-field-title"><?php _e( 'Keyword', ET_DOMAIN ); ?></label>
                                                        <input type="text" class="fieldechange  search" 																name="s"placeholder="<?php _e( 'Search projects by keywords', ET_DOMAIN ); ?>">   <!-- mene77  fieldechange -->
                                                    </div>
                                                </div>
                                                <div class="col-md-4 col-sm-6 col-xs-12">
                                                    <div class="fre-input-field">
                                            ...
                                            <div class="fre-current-table-rows" style="display: table-row-group;">
												...
												while ( $employer_current_project_query->have_posts() ) {
                                                    ...
                                                    <div class="fre-table-row">
                                                        ...
                                                        <div class="fre-table-col project-status-col<?php 
														if((($convert->project_status_view)==__( 'Active', 																ET_DOMAIN )) || 
															(($convert->project_status_view)==__( 'Resolved', 															ET_DOMAIN ))){
															echo " active-st";
														}																											if((($convert->project_status_view)==__( 'Disputed', 															ET_DOMAIN )) || 
														  (($convert->project_status_view)==__( 'Pending', 															  ET_DOMAIN )) || 
														  (($convert->project_status_view)==__( 'Draft', 															  ET_DOMAIN )) || 
														  (($convert->project_status_view)==__( 'Archive', 															   ET_DOMAIN )) || 
															(($convert->project_status_view)==__( 'Processing', ET_DOMAIN ))){
															echo " waiting-st";
														}																											if((($convert->project_status_view)==__( 'Reject', 																ET_DOMAIN )) || 
															(($convert->project_status_view)==__( 'Close', 																ET_DOMAIN ))){
															echo " reject-st";
														}	
														?>"><?php echo $convert->project_status_view; ?></div>
														<?php
														if ( $project_status == 'close' ) {
															...
															echo '<a class="probtnsabt" href="' . add_query_arg( array( 'workspace' => 1 ), $convert->permalink ) . '" target="_blank">' . __( 'Workspace', ET_DOMAIN ) . '</a>';
															echo '</div>';
														} else if ( $project_status == 'disputing' ) {
															echo '<div class="fre-table-col project-action-col">';
															echo '<a class="probtnsabt" href="' . add_query_arg( array( 'dispute' => 1 ), $convert->permalink ) . '" target="_blank">' . __( 'Dispute Page', ET_DOMAIN ) . '</a>';
															echo '</div>';
														} else if ( $project_status == 'publish' ) {
															echo '<div class="fre-table-col project-action-col">';
															echo '<a class="probtnsabt project-action" data-action="archive" data-project-id="' . $convert->ID . '">' . __( 'Archive', ET_DOMAIN ) . '</a>';
															echo '</div>';
														} else if ( $project_status == 'pending' ) {
															echo '<div class="fre-table-col project-action-col">';
															echo '<a class="probtnsabt" href="' . et_get_page_link( 'edit-project', array( 'id' => $convert->ID ) ) . '" target="_blank">' . __( 'Edit', ET_DOMAIN ) . '</a>';
															echo '</div>';
														} else if ( $project_status == 'draft' ) {
															echo '<div class="fre-table-col project-action-col project-action-two">';
															echo '<a class="probtnsabt" href="' . et_get_page_link( 'submit-project', array( 'id' => $convert->ID ) ) . '" target="_blank">' . __( 'Edit', ET_DOMAIN ) . '</a>';
															echo '<a class="probtnsabt project-action" data-action="delete" data-project-id="' . $convert->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
															echo '</div>';
															...
															echo '<div class="fre-table-col project-action-col project-action-two">';
															echo '<a class="probtnsabt" href="' . et_get_page_link( 'submit-project', array( 'id' => $convert->ID ) ) . '" target="_blank">' . __( 'Renew', ET_DOMAIN ) . '</a>';
															echo '<a class="probtnsabt project-action" data-action="delete" data-project-id="' . $convert->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
															echo '</div>';
														}
														?>
