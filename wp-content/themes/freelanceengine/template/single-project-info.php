            ...
            <div class="col-lg-8 col-md-7">
                ...
                <ul class="project-bid-info-list">
                    ...
                    <li>				<!--///  mene77  budget  ///-->
                        ...
                        <span class="secondary-color" ><?php echo $Budget_line; ?></span>
					...
					if ( is_user_logged_in() ) {
						if ( $project_status == 'publish' ) {
							if ( ( fre_share_role() || $user_role == FREELANCER ) && $user_ID != $project->post_author ) {
								...
								if ( $has_bid ) {
									echo '<a class="probtnsabt fre-normal-btn primary-bg-color bid-action" data-action="cancel" data-bid-id="' . $bidding_id . '">' . __( 'Cancel', ET_DOMAIN ) . '</a>';
								} else {
								...
								echo '<a class="probtnsabt probtnsabt fre-action-btn  project-action" data-action="archive" data-project-id="' . $project->ID . '">' . __( 'Archive', ET_DOMAIN ) . '</a>';
							} else {
								echo '<a href="' . et_get_page_link( 'submit-project' ) . '" class="probtnsabt fre-normal-btn primary-bg-color">' . __( 'Post Project Like This', ET_DOMAIN ) . '</a>';
							}
							...
							if ( (int) $project->post_author == $user_ID || $bid_accepted_author == $user_ID || current_user_can( 'manage_options' ) ) {
								echo '<a class="probtnsabt fre-normal-btn" href="' . add_query_arg( array( 'dispute' => 1 ), $project_link ) . '">' . __( 'Dispute Page', ET_DOMAIN ) . '</a>';
							}
						} else if ( $project_status == 'close' ) {
							$bid_accepted_author = get_post_field( 'post_author', $bid_accepted );
							if ( (int) $project->post_author == $user_ID || $bid_accepted_author == $user_ID ) {
								echo '<a class="probtnsabt fre-normal-btn" href="' . add_query_arg( array( 'workspace' => 1 ), $project_link ) . '">' . __( 'Workspace', ET_DOMAIN ) . '</a>';
							}
							...
							if ( (int) $project->post_author == $user_ID || $bid_accepted_author == $user_ID ) {
								...
								if ( $commission ) {
									if ( $order_status != 'finish' ) {
										echo '<a class="probtnsabt fre-normal-btn primary-bg-color manual-transfer" data-project-id="' . $project->ID . '">' . __( "Transfer Money", ET_DOMAIN ) . '</a>';
									} else {
							...
							if ( ( fre_share_role() || $user_role == EMPLOYER ) && $user_ID == $project->post_author ) {
								echo '<a class="probtnsabt fre-action-btn" href="' . et_get_page_link( 'edit-project', array( 'id' => $project->ID ) ) . '">' . __( 'Edit', ET_DOMAIN ) . '</a>';
							} else if ( current_user_can( 'manage_options' ) ) {
								echo '<a class="probtnsabt fre-normal-btn primary-bg-color project-action" data-action="approve" data-project-id="' . $project->ID . '">' . __( 'Approve', ET_DOMAIN ) . '</a>';
								echo '<a class="probtnsabt fre-normal-btn primary-bg-color project-action" data-action="reject" data-project-id="' . $project->ID . '">' . __( 'Reject', ET_DOMAIN ) . '</a>';
							}
							...
							if ( ( fre_share_role() || $user_role == EMPLOYER ) && $user_ID == $project->post_author ) {
								echo '<a class="probtnsabt fre-action-btn" href="' . et_get_page_link( 'submit-project', array( 'id' => $project->ID ) ) . '">' . __( 'Edit', ET_DOMAIN ) . '</a>';
								echo '<a class="probtnsabt fre-action-btn project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
							} else if ( current_user_can( 'manage_options' ) ) {
								echo '<a class="probtnsabt fre-action-btn project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
							}
						} else if ( $project_status == 'archive' ) {
							if ( ( fre_share_role() || $user_role == EMPLOYER ) && $user_ID == $project->post_author ) {
								echo '<a class="probtnsabt fre-action-btn" href="' . et_get_page_link( 'submit-project', array( 'id' => $project->ID ) ) . '">' . __( 'Renew', ET_DOMAIN ) . '</a>';
								echo '<a class="probtnsabt fre-action-btn project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
							} else if ( current_user_can( 'manage_options' ) ) {
								echo '<a class="probtnsabt fre-action-btn project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __( 'Delete', ET_DOMAIN ) . '</a>';
							}
						...
						if ( $project_status == 'publish' ) {
							echo '<a class="probtnsabt fre-normal-btn primary-bg-color" href="' . et_get_page_link( 'login', array( 'ae_redirect_url' => $project->permalink ) ) . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
						}
					}
					?>
                </div>
