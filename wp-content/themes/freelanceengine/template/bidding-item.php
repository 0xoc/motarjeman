...
<div class="row freelancer-bidding-item">
			...
			if ( $user_ID == $project->post_author ) {
				if ( $convert->flag == 1 ) {
					if ( ae_get_option( 'use_escrow' ) ) {
						echo '<a id="' . get_the_ID() . '" rel="' . $project->ID . '" class="probtnsabt fre-normal-btn btn-accept-bid">' . __( 'Accept Bid', ET_DOMAIN ) . '</a>';
					} else {
			/* mene77 */	echo '<a class="probtnsabt fre-normal-btn btn-accept-bid btn-accept-bid-no-escrow" id="' . get_the_ID() . '">' . __( 'Accept Bid', ET_DOMAIN ) . '</a>';
					}
				}
	...
	<?php if ( $show_bid_info ) { ?>
		<?php do_action( 'after_bidding_item', $convert, $project ); ?>
	<?php } ?>
</div>